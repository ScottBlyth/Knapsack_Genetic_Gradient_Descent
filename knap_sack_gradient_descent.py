...
def loss(vec, weights,values, cap, l1,l2, l3, t_v):
...
def knap_sack_grad(weights, values, cap, l1, l2, l3,t_v,learning_rate,iterations=1000):
    k = len(weights)
    vec = [0 for _ in range(k)]
    print(loss(vec, weights,values, cap, l1,l2, l3, t_v))
    for i in range(iterations):
        ...
        for j in range(k):
            ...
            part_2 = p*(1-p)*values[j]*(total_value-t_v)
            part_3 = p*(1-p)*(p-1)
